...
def setup_content(db_path: Path) -> tuple[int, int, int, int, int]:
    ...
    try:
        init_schema(conn)
        parent_id = q.add_folder(conn, "Parent")
        child_id = q.add_folder(conn, "Child", parent_id)
        parent_feed_id = q.add_feed(
            conn,
...
def test_sidebar_tree_aggregates_nested_unread_counts(db_path: Path) -> None:
    ...
    conn = connect(db_path)
    try:
        tree, orphan_feeds, orphan_unread = q.build_sidebar_tree(
            q.list_folders(conn), q.list_feeds(conn)
        )
    finally:
        conn.close()
...
def test_list_entries_filters_by_folder_descendants_and_unread(db_path: Path) -> None:
    ...
    conn = connect(db_path)
    try:
        rows = q.list_entries(conn, scope="folder", folder_id=parent_id, unread_only=True)
    finally:
        conn.close()
...
def test_list_feeds_filtered_can_select_only_orphans(db_path: Path) -> None:
    *_, orphan_feed_id = setup_content(db_path)
    conn = connect(db_path)
    try:
        rows = q.list_feeds_filtered(conn, folder_ids=[], include_orphan=True)
    finally:
        conn.close()
...
def test_mark_read_and_toggle_star_update_entry_state(db_path: Path) -> None:
    ...
    try:
        ...
        q.mark_read(conn, entry_id, True)
        assert (
            conn.execute(
        ...
        assert q.toggle_star(conn, entry_id) is True
        row = conn.execute(
